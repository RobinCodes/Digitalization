...
We understand that it should exist, since in a right triangle with legs $a = 1, b= 1, \alpha = 90$, where $\alpha$ is their enclosed angle, the hypotenuse has length $\sqrt{2}$. So what is $\sqrt{2}$?

\begin{theorem}
    There is no rational number $x$ such that $x^2 = 2$. Equivalently, for an x satisfying $x^2 = 2$, x may not also satisfy $x = \frac{p}{q}$ with $p, q \in \mathbb{Z}$, $\gcd(p, q) = 1$
\end{theorem}
\begin{proof}
    ...
    \end{equation}
    Or alternatively, we could have also finished the proof via noticing that in the equation
    $p^2 = 2q^2$, $p^2$ has two in its prime factorization to an even power (since it is a square), while $2q^2$ has two in its prime factorization to an odd power (since it is a square, but multiplied by 2). Therefore our equation cannot hold, since one number cannot have two in its prime factorization to an odd and an even power at the same time.
\end{proof}
...
\begin{remark}[Proof of irrationality of the square root of non-square numbers]
If $x^2 = a$ and $a$ is not a perfect square, then and only then $x \in \mathbb{R} \setminus \mathbb{Q}$.
    \begin{proof}
        If $a$ is not a perfect square, then $a$ contains a prime factor to an odd exponent. Then, if $a$ were rational, we would have $a = \frac{p}{q}$ for some $p, q \in \mathbb{Z}$. (Note that, without loss of generality, we could also assume that $q > 0$ and that $\gcd(p, q) = 1$.)

        Then, since $a = \frac{p}{q}$, $\frac{p^2}{q^2} = a \implies p^2 = a\cdot q^2$.\\
        Now, pick a prime factor in $a$ whose exponent is odd. (We know one must exist, since $a$ is not a perfect square.)
        \begin{itemize}
            \item in the prime factorization of $p$, $r$ must appear an even number of times, since it is a perfect square.
            \item in the prime factorization of $aq^2$, $r$ must appear an odd numbers of times, since the above argument applies to $q^2$, and multiplying that once by $a$ yields an odd exponent.
        \end{itemize}
        We have reached a contradiction, as no number can have a prime in its prime factorization to an even and an odd power at the same time, and prime factorizations are unique by the Fundamental Theorem of Arithmetic. Therefore $a \in \mathbb{R} \setminus \mathbb{Q}.$
        
    \end{proof}
\end{remark}

\section{Finding $\sqrt{2}$}
Since we cannot find irrational numbers exactly, we will approximate it. Let us start with:
\[
...
Now we proceed with a proof of Theorem 2 using the distributivity of multiplication:
...
\begin{proof}
    ...
    xy < y^2
    \]
    Combining the latter two under the Axiom of Transitivity yields:
    \[
...
\section{Lion Catching Method}
We can find $\sqrt{2}$ algorithmically via the \textbf{Bisection Method}, also known as the \emph{Interval Halving Method}, or the \emph{Lion Catching Method.} (from \emph{Hector Petard: A Contribution to the Mathematical Theory of Big Game Hunting, 1938.)}\\
Given $[a_0, b_0]$, $c_0 = \frac{a_0 + b_0}{2}$ is the arithmetic mean, since
\[
...
We will give the algorithm a limit, so that the program halts and we can retrieve the calculation. This limit will be its \emph{precision}. (Alternatively, we could make the program report on its progress while storing the previous value in memory for a little longer, and continue [in theory] indefinitely.)
...
\end{proposition}
However, we will see that this is actually not the right way to look at the problem, since, as we will see, "any arbitrary day" is not equivalent to infinity. Instead, we observe the following proposition:
\begin{proposition}
    He can steal all of them, therefore there doesn't exist gold that he has not stolen.
\end{proposition}
...
\begin{theorem}[Inverse of a Bijection]
    A function $f: A \rightarrow B$ is a bijection if and only if there exists a function $g: B \rightarrow A$ (often denoted as $f^{-1}$) such that:
    \begin{enumerate}
        \item $g(f(a)) = a$ for all $a \in A$ (equivalently, $g \circ f = \text{id}_A$)
        \item $f(g(b)) = b$ for all $b \in B$ (equivalently, $f \circ g = \text{id}_B$)
    \end{enumerate}
\end{theorem}

\begin{proof}
    \textbf{Part 1: Forward Direction ($\implies$)}\\
    Assume $f: A \rightarrow B$ is a bijection. We must show that an inverse function $g: B \rightarrow A$ exists.
    \begin{itemize}
        \item Because $f$ is \textbf{surjective}, for every element $b \in B$, there is at least one element $a \in A$ such that $f(a) = b$.
        \item Because $f$ is \textbf{injective}, this element $a$ is strictly unique. There is only one $a$ that maps to a specific $b$.
        \item Therefore, we can unambiguously define a new function $g: B \rightarrow A$ by setting $g(b) = a$.
        \item By this definition, applying $f$ to $g(b)$ yields $f(g(b)) = f(a) = b$ for all $b \in B$. 
        \item Similarly, applying $g$ to $f(a)$ yields $g(f(a)) = g(b) = a$ for all $a \in A$. 
        \item Thus, the inverse function $g$ exists and satisfies both conditions.
    \end{itemize}

    \vspace{1em} % Adds a little visual breathing room between parts
    \textbf{Part 2: Reverse Direction ($\impliedby$)}\\
    Assume there exists a function $g: B \rightarrow A$ such that $g(f(a)) = a$ and $f(g(b)) = b$. We must show that $f$ is a bijection (both injective and surjective).
    \begin{itemize}
        \item \textbf{Injective:} Suppose $f(a_1) = f(a_2)$ for some $a_1, a_2 \in A$.
        Applying our function $g$ to both sides gives:
        \[
            g(f(a_1)) = g(f(a_2))
        \]
        Because we know $g(f(a)) = a$, this equation simplifies directly to:
        \[
            a_1 = a_2
        \]
        Since $f(a_1) = f(a_2) \implies a_1 = a_2$, the function $f$ is injective.

        \item \textbf{Surjective:} Let $b$ be an arbitrary element in $B$. We need to find an element $a \in A$ such that $f(a) = b$.
        Let $a = g(b)$.
        Applying $f$ to this chosen $a$ gives:
        \[
            f(a) = f(g(b))
        \]
        Because we know $f(g(b)) = b$, we have:
        \[
            f(a) = b
        \]
        Therefore, for every $b \in B$, there is an $a \in A$ that maps to it, making $f$ surjective.
    \end{itemize}
    
    Since $f$ is proven to be both injective and surjective, it is, by definition, a bijection.
\end{proof}
...
\begin{definition}[Uncountably infinite set]
    ...
    The classic example is $\mathbb{R}$, proved by \textbf{Cantor's diagonalization argument.}
\end{definition}

With these concepts in mind now, let us reconsider the original gold problem.

\begin{proof}
Since the set of all gold bars is $\mathbb{N}$ (as they can be organised next to each other, like listing them), and the set of all days is also $\mathbb{N}$ since we can also "list them out", both sets are countably infinite, and thus their cardinalities are equal. This tells us that the vault \emph{can be exhausted.} (Note that this proof is not "rigorous" in any mathematical sense, as the question is also not posed in a mathematical sense. Refer to \emph{Cantor's diagonalization argument.})
\end{proof}
...
\begin{definition}[Principal Square Root]
    We denote the principal square root via the symbol $\sqrt{\phantom{x}}$. To ensure this operation is a well-defined function, we restrict its codomain to non-negative real numbers. Thus,
    \[
        \sqrt{x} = a \iff a^2 = x
    \]
    where $a \in \mathbb{R}$ and $a \ge 0$.
\end{definition}

\begin{remark}[Why not "Without Loss of Generality"?]
    We cannot state $a \ge 0$ "without loss of generality." The equation $a^2 = x$ (for $x > 0$) has two distinct real solutions. If we assumed $a \ge 0$ without loss of generality, it would imply that choosing $a \le 0$ yields the exact same mathematical properties. Instead, restricting $a \ge 0$ is a strict definitional choice required to make the square root a single-valued function (specifically, the \textbf{principal} branch).
\end{remark}
...
\]
Generally, $a\sqrt{b} = \sqrt{a^2b}.$ and $\sqrt{c} = m\sqrt{n}$ where $c = m^2n.$ (Every number is of this form, however, $m$ may be $1$ for some cases, therefore this may not lead to an actual reduction.)\\
